...
public:
    ...
    void rcvTrajectoryCallabck(const quadrotor_msgs::PolynomialTrajectory & traj)
    {
        ...
        {
            ROS_WARN("[SERVER] Aborting the trajectory."); //终止轨迹
            state = HOVER;
            _traj_flag = quadrotor_msgs::PositionCommand::TRAJECTORY_STATUS_COMPLETED;
        }
    ...
    void pubPositionCommand()
    {
        ...
        if (state == TRAJ)
        {
            ...
            for (int idx = 0; idx < _n_segment; ++idx)
            {
                ...
                {   
                        ...
                        for(int i = 0; i < cur_poly_num; i ++)  // col 列
                        {
        ...
        tf::Quaternion q_ = tf::createQuaternionFromYaw(_cmd.yaw); //此处将航向角转化为四元数
        ...
        pt.x = _cmd.position.x + _cmd.velocity.x;  /**********************************此处运算不理解***************************************/
        ...
        pt.x = _cmd.position.x + _cmd.acceleration.x;  /**********************************此处运算不理解***************************************/
