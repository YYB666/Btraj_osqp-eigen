...
void rcvOdometryCallbck(const nav_msgs::Odometry odom)
{
    ...
    //2，创建一个tf对象 （定义存放转换信息，包括平移旋转信息）
    tf::Transform transform;
    //3，设置tf的平移旋转
    //设置坐标原点
    transform.setOrigin( tf::Vector3(_odom.pose.pose.position.x, _odom.pose.pose.position.y, _odom.pose.pose.position.z) ); //child node 相对于 parent node ，变换方向为 parent node ----> child node
    //设置这个子坐标系与父坐标系的角度关系，前三个参数为pitch,row,yaw,最后一个参数为角速度
    transform.setRotation(tf::Quaternion(0, 0, 0, 1.0)); //no rotation ，pitch=row=yaw=0
    //4，发布tf变换
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "world", "quadrotor")); // "world"此处为 parent node , "quadrotor" 为 child node 
}
...
*/       
    Cube cube;
    //  pt(0)>_pt_max_x, pt(0)=_pt_max_x
    //  pt(0)<_pt_min_x, pt(0)=_pt_min_x
    //  _pt_min_x<=pt(0)<=_pt_max_x, pt(0)=pt(0)
...
bool isContains(Cube cube1, Cube cube2)
{   
    //判断Cube1 是否包含 cube2，及 cube2 是否位于 cube1 内
...
//此处为减少计算量，提高计算速度，使用“抛物线+直线+抛物线”的组合方式来简化实现双曲正切函数“f(d)=max_v*(tanh(d-e)+1)/2”,e 为 Euler's number
...
{   
    ...
    if(_is_use_fm)   //此参数由参数服务器决定
    {
        ros::Time time_1 = ros::Time::now();

        /***************** Build the signed distance field ************************/
        float oob_value = INFINITY;  //无穷         
        auto EDT = collision_map_local->ExtractDistanceField(oob_value); //Euclidean distance transform (EDT)
        //此处 EDT 即为 the signed distance field
        ...
        for(unsigned int k = 0; k < size_z; k++)
        {
            for(unsigned int j = 0; j < size_y; j++)
            {
                for(unsigned int i = 0; i < size_x; i++)
                {
                    ...
                    if(collision_map_local->Inside(index))  //判断 index 是否超出 collision_map_local 的索引范围
                    {
                        //从 EDT 场中读取 距离值 d
        ...
        grid_fmm[goalIdx].setOccupancy(max_vel); /********************* 此处不懂 **************************/    
        ...
        if(fm_solver->compute(max_vel) == -1)  // fm_solver->compute(max_vel) 此函数内部调用 FMM::computeInternal() 完成FMM 
        {
    ...
    ros::Time time_bef_opt = ros::Time::now();
     ...
     /***后端的轨迹优化在 _trajectoryGenerator.BezierPloyCoeffGeneration（ ）此函数中完成，包括 约束条件的创立 以及建立 MOSEK 工作环境 ，使用 MOSEK 求解优化问题****/
     /**************此函数调用完成后，_bezier_coeff 存储最后优化所得的轨迹参数************/
    ...
    {   
        _seg_num = corridor.size();  //获取轨迹段数
        _seg_time.resize(_seg_num);

        for(int i = 0; i < _seg_num; i++)
            _seg_time(i) = corridor[i].t;  //給每段軌跡 賦值時間
...
{   
    ...
    for(int i = 0; i < (int)points.size() - 1; i++)
    {
        ...
        double D    = d.norm();       //返回  Vector3d d  的二范数          
...
int main(int argc, char** argv)
{
    ...
    //平移
    Translation3d origin_translation( _map_origin(0), _map_origin(1), 0.0);//左下角
    //旋转（四元数表示法）
    //Quaternion (const Scalar &w, const Scalar &x, const Scalar &y, const Scalar &z)
    //w是四元数的实部, x,y,z 分别是四元数的虚部；但是 ！！！在实际的内存中, 四元数四个元素的顺序却是 x, y, z, w！！！
    Quaterniond origin_rotation(1.0, 0.0, 0.0, 0.0);
    //仿射变换(Affine transformations) 平移*旋转*放缩，此处只有前两项
    ...
    while(status) 
    {
        //ros::spinOnce()时，就会执行callback函数（只调用一次），当spinOnce()被调用时，调用队列中第1个数据作为callback函数的参数；等到下次spinOnce()被调用时，队列中第2个数据被使用，以此类推
        ros::spinOnce();           
        status = ros::ok();
        rate.sleep();
    }
...
quadrotor_msgs::PolynomialTrajectory getBezierTraj()
{
    quadrotor_msgs::PolynomialTrajectory traj;
      ...
      traj.num_segment = _seg_num;  //该参数由走廊个数决定

      int order = _traj_order;  //该参数由参数服务器决定
      int poly_num1d = order + 1;
      int polyTotalNum = _seg_num * (order + 1);

      traj.coef_x.resize(polyTotalNum);  //traj.coef_x 类型为 vector ,存储X轴向所有段轨迹的系数
      ...
      {    
          ...
          { 
              traj.coef_x[idx] = _bezier_coeff(i,                  j);  //矩陣 _bezier_coeff 每一行依次存儲每段 X Y Z 系數
